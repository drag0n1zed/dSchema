...
public class SchemaNotAnOperatorScreen extends AbstractPanelScreen {

    public SchemaNotAnOperatorScreen(Entrance entrance) {
        super(entrance, Text.empty(), PANEL_WIDTH_60, 0);
    }

    @Override
    public void onCreate() {
        addWidget(new TextWidget(getEntrance(), getLeft() + getWidth() / 2, getTop() + PANEL_TITLE_HEIGHT_1 - 10, Text.translate("dschema.not_an_operator.title").withColor(AbstractPanelScreen.TITLE_COLOR), TextWidget.Gravity.CENTER));
    ...
    private List<Text> getMessages() {
        return TooltipHelper.wrapLines(getTypeface(), Text.translate("dschema.not_an_operator.message"), getWidth() - PADDINGS_H * 2);
    }
