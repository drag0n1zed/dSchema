...
import io.github.drag0n1zed.universal.api.gui.slot.TextSlot;
...
public class SchemaTransformerEditScreen extends AbstractPanelScreen {
...
//    private EditBox nameEditBox;
    ...
    public SchemaTransformerEditScreen(Entrance entrance, Consumer<Transformer> consumer, Transformer transformer) {
        super(entrance, Text.translate("dschema.transformer.edit.title"), PANEL_WIDTH_60, PANEL_HEIGHT_FULL);
...
//        this.nameEditBox.setHint(Text.translate("dschema.transformer.randomizer.edit.name_hint"));
        ...
        this.moveToPlayerButton = addWidget(Button.builder(getEntrance(), Text.translate("dschema.position.move_to_player"), button -> {
        ...
        this.moveToCornerOrCenterButton = addWidget(Button.builder(getEntrance(), Text.translate("dschema.position.move_to_center_or_corner"), button -> {
            this.transformer = switch (transformer.getType()) {
                case ARRAY -> transformer;
        ...
        this.cancelButton = addWidget(Button.builder(getEntrance(), Text.translate("dschema.transformer.edit.cancel"), button -> {
            detach();
        }).setBoundsGrid(getLeft(), getTop(), getWidth(), getHeight(), 0f, 0f, 0.5f).build());
        this.saveButton = addWidget(Button.builder(getEntrance(), Text.translate("dschema.transformer.edit.save"), button -> {
        ...
        switch (transformer.getType()) {
            case ARRAY -> {
                this.entries.addIntegerEntry(Text.translate("dschema.transformer.array.offset.x"), Text.text("X"), ((ArrayTransformer) transformer).offset().x(), ArrayTransformer.OFFSET_BOUND.minX(), ArrayTransformer.OFFSET_BOUND.maxX(), value -> this.transformer = ((ArrayTransformer) transformer).withOffsetX(value));
                this.entries.addIntegerEntry(Text.translate("dschema.transformer.array.offset.y"), Text.text("Y"), ((ArrayTransformer) transformer).offset().y(), ArrayTransformer.OFFSET_BOUND.minY(), ArrayTransformer.OFFSET_BOUND.maxY(), value -> this.transformer = ((ArrayTransformer) transformer).withOffsetY(value));
                this.entries.addIntegerEntry(Text.translate("dschema.transformer.array.offset.z"), Text.text("Z"), ((ArrayTransformer) transformer).offset().z(), ArrayTransformer.OFFSET_BOUND.minZ(), ArrayTransformer.OFFSET_BOUND.maxZ(), value -> this.transformer = ((ArrayTransformer) transformer).withOffsetZ(value));
                this.entries.addIntegerEntry(Text.translate("dschema.transformer.array.count"), Text.text("C"), ((ArrayTransformer) transformer).count(), ArrayTransformer.COUNT_RANGE.min(), ArrayTransformer.COUNT_RANGE.max(), value -> this.transformer = ((ArrayTransformer) transformer).withCount(value));
            }
            case MIRROR -> {
                ...
                this.entries.addSelectorEntry(Text.translate("dschema.transformer.mirror.axis"), Text.text("A"), Arrays.stream(Axis.values()).map(Axis::getDisplayName).toList(), Arrays.stream(Axis.values()).toList(), ((MirrorTransformer) transformer).axis(), value -> this.transformer = ((MirrorTransformer) transformer).withAxis(value));
                this.entries.addIntegerEntry(Text.translate("dschema.transformer.mirror.size"), Text.text("S"), ((MirrorTransformer) transformer).size(), MirrorTransformer.SIZE_RANGE.min(), MirrorTransformer.SIZE_RANGE.max(), value -> this.transformer = ((MirrorTransformer) transformer).withSize(value));
            }
            case RADIAL -> {
                ...
                this.entries.addSelectorEntry(Text.translate("dschema.transformer.radial.axis"), Text.text("A"), Arrays.stream(Axis.values()).map(Axis::getDisplayName).toList(), Arrays.stream(Axis.values()).toList(), ((RadialTransformer) transformer).axis(), value -> this.transformer = ((RadialTransformer) transformer).withAxis(value));
                this.entries.addIntegerEntry(Text.translate("dschema.transformer.radial.slices"), Text.text("S"), ((RadialTransformer) transformer).slices(), RadialTransformer.SLICE_RANGE.min(), RadialTransformer.SLICE_RANGE.max(), value -> this.transformer = ((RadialTransformer) transformer).withSlice(value));
                this.entries.addIntegerEntry(Text.translate("dschema.transformer.radial.radius"), Text.text("R"), ((RadialTransformer) transformer).radius(), RadialTransformer.RADIUS_RANGE.min(), RadialTransformer.RADIUS_RANGE.max(), value -> this.transformer = ((RadialTransformer) transformer).withRadius(value));
                this.entries.addIntegerEntry(Text.translate("dschema.transformer.radial.length"), Text.text("L"), ((RadialTransformer) transformer).length(), RadialTransformer.LENGTH_RANGE.min(), RadialTransformer.LENGTH_RANGE.max(), value -> this.transformer = ((RadialTransformer) transformer).withLength(value));
            }
            case RANDOMIZER -> {
            }
        }
