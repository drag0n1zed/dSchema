...
import io.github.drag0n1zed.universal.api.gui.text.MessageTextWidget;
...
public class SchemaClipboardScreen extends AbstractPanelScreen {
...
//    private EditBox nameEditBox;
    ...
    public SchemaClipboardScreen(Entrance entrance) {
        super(entrance, Text.translate("dschema.clipboard.edit.title"), PANEL_WIDTH_60, PANEL_HEIGHT_FULL);
...
//        this.enableButton = addWidget(Button.builder(getEntrance(), clipboard.enabled() ? Text.translate("dschema.clipboard.button.disable") : Text.translate("dschema.clipboard.button.enable"), button -> {
...
//            new dschemaClipboardHistoryScreen(getEntrance(), snapshot -> {
...
//        }).setBounds(this.snapshotWidget.getRight() - Button.DEFAULT_HEIGHT - 2, this.snapshotWidget.getTop() + 2 * 4 + Button.DEFAULT_HEIGHT * 3, Button.DEFAULT_HEIGHT, Button.DEFAULT_HEIGHT).build());
        ...
        this.historyButton = addWidget(Button.builder(getEntrance(), Text.translate("dschema.button.history"), button -> {
            new SchemaStructureSnapshotsHistoryScreen(getEntrance(), snapshot -> {
                this.clipboard = this.clipboard.withSnapshot(snapshot);
            }).attach();
        }).setBoundsGrid(getLeft(), getTop(), getWidth(), getHeight(), 1f, 0 / 4f, 1 / 4f).build());

//        this.libraryButton = addWidget(Button.builder(getEntrance(), Text.translate("dschema.button.library"), button -> {
//            new dschemaStructureSnapshotsLibraryScreen(getEntrance(), snapshot -> {
//                this.clipboard = this.clipboard.withSnapshot(snapshot);
//            }).attach();
//        }).setBoundsGrid(getLeft(), getTop(), getWidth(), getHeight(), 1f, 1 / 4f, 1 / 4f).build());

        this.doneButton = addWidget(Button.builder(getEntrance(), Text.translate("dschema.button.done"), button -> {
            detachAll();
    ...
    public void onReload() {
        ...
        this.historyButton.setTooltip(TooltipHelper.makeSummary(getTypeface(), Text.translate("dschema.button.history"), Text.translate("dschema.clipboard.history.tooltip")));

//        this.libraryButton.setTooltip(TooltipHelper.makeSummary(getTypeface(), Text.translate("dschema.button.library"), Text.translate("dschema.clipboard.library.tooltip")));

        this.shareButton.setTooltip(TooltipHelper.makeSummary(getTypeface(), Text.translate("dschema.button.share"), Text.translate("dschema.clipboard.share.tooltip")));
        this.shareButton.setActive(!clipboard.isEmpty());

        this.clearButton.setTooltip(TooltipHelper.makeSummary(getTypeface(), Text.translate("dschema.button.clear"), Text.translate("dschema.clipboard.clear.tooltip")));
        this.clearButton.setActive(!clipboard.isEmpty());

        this.editButton.setTooltip(TooltipHelper.makeSummary(getTypeface(), Text.translate("dschema.button.edit"), Text.translate("dschema.clipboard.edit.tooltip")));
        this.editButton.setActive(!clipboard.isEmpty());

        this.enableButton.setTooltip(TooltipHelper.makeSummary(getTypeface(), clipboard.enabled() ? Text.translate("dschema.clipboard.button.disable") : Text.translate("dschema.clipboard.button.enable"), Text.translate("dschema.clipboard.enable_clipboard.tooltip", OptionKeys.KEY_ATTACK.getKeyBinding().getKey().getNameText(), OptionKeys.KEY_USE.getKeyBinding().getKey().getNameText())));
