...
public class SchemaSessionStatusScreen extends AbstractPanelScreen {

    public SchemaSessionStatusScreen(Entrance entrance) {
    ...
    public void onCreate() {
        addWidget(new TextWidget(getEntrance(), getLeft() + getWidth() / 2, getTop() + PANEL_TITLE_HEIGHT_1 - 10, Text.translate("dschema.session_status.title").withColor(AbstractPanelScreen.TITLE_COLOR), TextWidget.Gravity.CENTER));
    ...
    private List<Text> getMessages() {
        return TooltipHelper.wrapLines(getTypeface(),
                switch (getEntrance().getSessionManager().getSessionStatus()) {
                    ...
                    case PROTOCOL_NOT_MATCH -> Text.translate("dschema.session_status.message.protocol_not_match", getEntrance().getSessionManager().getServerSession().protocolVersion(), getEntrance().getSessionManager().getLastSession().protocolVersion());
                    case SUCCESS -> Text.translate("dschema.session_status.message.success", getEntrance().getSessionManager().getServerSession().loaderType().name());
