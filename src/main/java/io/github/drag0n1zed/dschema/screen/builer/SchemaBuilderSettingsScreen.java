package io.github.drag0n1zed.dschema.screen.builer;
...
import io.github.drag0n1zed.dschema.building.config.BuilderConfig;
...
public class SchemaBuilderSettingsScreen extends AbstractPanelScreen {

    private final Consumer<ClientConfig> consumer;
    private ClientConfig originalConfig;
    private ClientConfig config;

    public SchemaBuilderSettingsScreen(Entrance entrance) {
        super(entrance, Text.translate("dschema.builder_settings.title"), PANEL_WIDTH_60, PANEL_HEIGHT_FULL);
    ...
    public void onCreate() {
        ...
        entries.addSwitchEntry(Text.translate("dschema.render_settings.show_block_preview"), null, this.config.renderConfig().showBlockPreview(), (value) -> {
        ...
        entries.addSwitchEntry(Text.translate("dschema.render_settings.show_other_players_build"), null, this.config.renderConfig().showOtherPlayersBuild(), (value) -> {
            var renderConfig = this.config.renderConfig();
...
//        entries.addSwitchEntry(Text.translate("dschema.render_settings.show_other_players_build_tooltips"), null, config.showOtherPlayersBuildTooltips(), (value) -> {
...
//        });
        entries.addIntegerEntry(Text.translate("dschema.render_settings.max_render_volume"), null, this.config.renderConfig().maxRenderVolume(), RenderConfig.MAX_RENDER_VOLUME_MIN, RenderConfig.MAX_RENDER_VOLUME_MAX, (value) -> {
            var renderConfig = this.config.renderConfig();
...
//        entries.addIntegerEntry(Text.translate("dschema.render_settings.max_render_distance"), null, config.maxRenderDistance(), RenderConfig.MIN_MAX_RENDER_DISTANCE, RenderConfig.MAX_MAX_RENDER_DISTANCE, (value) -> {
...
//        });
        ...
        entries.addIntegerEntry(Text.translate("dschema.builder_settings.reserved_tool_durability"), null, this.config.builderConfig().reservedToolDurability(), BuilderConfig.RESERVED_TOOL_DURABILITY_RANGE.min(), BuilderConfig.RESERVED_TOOL_DURABILITY_RANGE.max(), (value) -> {
            this.config = this.config.withBuilderConfig(new BuilderConfig(value, this.config.builderConfig().passiveMode()));
        });
        entries.addSwitchEntry(Text.translate("dschema.builder_settings.passive_mode"), null, this.config.builderConfig().passiveMode(), (value) -> {
            this.config = this.config.withBuilderConfig(new BuilderConfig(config.builderConfig().reservedToolDurability(), value));
        });

        entries.addTab(Text.translate("dschema.pattern_settings.item_randomizer_presets"), null, config.patternConfig().transformerPreset(), (value) -> {
