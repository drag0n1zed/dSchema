...
import io.github.drag0n1zed.dschema.renderer.BlockShaders;
import io.github.drag0n1zed.dschema.renderer.opertaion.OperationsRenderer;
import io.github.drag0n1zed.dschema.renderer.outliner.OutlineRenderer;
import io.github.drag0n1zed.dschema.renderer.pattern.PatternRenderer;
import io.github.drag0n1zed.dschema.renderer.tooltip.TooltipRenderer;
...
import io.github.drag0n1zed.dschema.screen.structure.SchemaStructureScreen;
import io.github.drag0n1zed.dschema.screen.test.SchemaTestScreen;
...
public final class SchemaClientManager implements ClientManager {
    ...
    public void onKeyInput(InputKey key) {
        ...
        if (SchemaKeys.TOGGLE_CLIPBOARD.getKeyBinding().consumeClick()) {
            ...
            getEntrance().getClient().getPlayer().sendMessage(Schema.getSystemMessage(Text.translate("dschema.message.building.server.toggle_clipboard", getEntrance().getStructureBuilder().getContext(getRunningClient().getPlayer()).clipboard().getNameText().withStyle(ChatFormatting.GOLD))));
        }
        if (SchemaKeys.TOGGLE_PATTERN.getKeyBinding().consumeClick()) {
            ...
            getEntrance().getClient().getPlayer().sendMessage(Schema.getSystemMessage(Text.translate("dschema.message.building.server.toggle_pattern", getEntrance().getStructureBuilder().getContext(getRunningClient().getPlayer()).pattern().getNameText().withStyle(ChatFormatting.GOLD))));
        }
        if (SchemaKeys.TOGGLE_REPLACE.getKeyBinding().consumeClick()) {
            ...
            getEntrance().getClient().getPlayer().sendMessage(Schema.getSystemMessage(Text.translate("dschema.message.building.server.toggle_replace", getEntrance().getStructureBuilder().getContext(getRunningClient().getPlayer()).replace().getNameText().withStyle(ChatFormatting.GOLD))));
        }
...
//        }
        ...
        if (Platform.getInstance().isDevelopment()) {
            if (Keys.KEY_LEFT_CONTROL.isDown() && Keys.KEY_ENTER.isDown()) {
                getEntrance().getClient().getSoundManager().playButtonClickSound();
                new SchemaTestScreen(getEntrance()).attach();
            }
        }
    }
    ...
    public void onRenderGui(Renderer renderer, float deltaTick) {
        if (getRunningClient().getPanel() != null && !(getRunningClient().getPanel() instanceof SchemaStructureScreen)) {
            return;
        }
