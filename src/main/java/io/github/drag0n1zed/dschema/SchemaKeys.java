...
import io.github.drag0n1zed.universal.api.input.KeyCodes;

public enum SchemaKeys implements KeyBindingOwner {
    // default
...
//    MIRROR_XYZ("mirror_xyz", KeyCodes.KEY_UNKNOWN, Category.CLIPBOARD),
    ...
    SchemaKeys(String description, Category category, KeyCodes defaultKey) {

        this.keyBinding = switch (category) {
            case DEFAULT ->  KeyBinding.of("key.dschema.%s.desc".formatted(description), "key.dschema.category.default", defaultKey.value());
            case CLIPBOARD ->  KeyBinding.of("key.dschema.%s.desc".formatted(description), "key.dschema.category.clipboard", defaultKey.value());
        };
    }
